...
fn copy_preview_assets() {
    ...
    println!("cargo:rerun-if-changed=scripts/custom-views-assets.mjs");
    println!("cargo:rerun-if-changed=node_modules/braintrust/dist");
    let status = Command::new("node")
        .current_dir(&manifest_dir)
        .arg("scripts/custom-views-assets.mjs")
        .arg(out_dir.join("custom-views.mjs"))
        .status()
        .expect("failed to run node to bundle custom view SDK assets");
    assert!(
        status.success(),
        "failed to bundle custom view SDK assets; run `pnpm install --ignore-scripts` before building bt"
    );
