...
function installDiscoveryReactStub() {
  ...
  globalThis.React = {
    ...
    lazy: (loader) => ({ $$typeof: Symbol.for("react.lazy"), _loader: loader }),
    memo: (type, compare = null) => ({
...
function validateCustomView(value, sourceFile) {
  ...
  if (typeof value.slug !== "string" || !value.slug.trim()) {
    throw new Error(
      `${sourceFile} custom view '${value.name}' slug is required`,
    );
  }
  if (
    ...
    ].includes(value.component?.$$typeof)
  ) {
    throw new Error(
      `${sourceFile} custom view '${value.slug}' component must be a React component`,
    );
  }
}
...
function datasetFields(view) {
  ...
  const fields = {};
  if (typeof view.dataset.id === "string" && view.dataset.id.trim()) {
    fields.dataset_id = view.dataset.id;
  }
  if (typeof view.dataset.name === "string" && view.dataset.name.trim()) {
    fields.dataset_name = view.dataset.name;
  }
  return fields;
}
...
async function buildManifest(inputPath) {
  ...
  for (const file of input.files) {
    ...
    if (typeof sourceFile !== "string" || typeof bundleFile !== "string") {
      throw new Error(
        "views metadata input entries require source_file and bundle_file",
      );
    }
