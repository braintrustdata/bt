...
// Installation runs before the Rust CLI exists, so clap cannot own these
// download opt-outs. The launcher also honors BT_BINARY_PATH.
...
if (process.env.BT_BINARY_PATH) {
  console.log(
    "bt: skipping post-install binary download because BT_BINARY_PATH is set.",
  );
  process.exit(0);
}
...
try {
  helper.resolvePackageBinary(packageName, subpath);
  // Optional dependency was installed successfully. Nothing to do.
  process.exit(0);
} catch (err) {
  // Fall through to the manual download path below.
  console.log(
    `bt: failed to locate the matching "${packageName}" package after installation: ${err.message}
...
if (!version) {
  console.error(
    `bt: cannot determine which version of "${packageName}" to download — it is not listed in the "@braintrust/bt" package's optionalDependencies. The bt CLI will not be available.`,
  );
  process.exit(1);
}
...
function extractFileFromTarball(tarball, target) {
  ...
  while (offset + 512 <= tarball.length) {
    ...
    if (
      ...
      fileSize < 0 ||
      offset + fileSize > tarball.length
    ) {
      throw new Error(`Invalid or truncated tar entry: ${fileName}`);
    }
    if (fileName === target) {
      return tarball.subarray(offset, offset + fileSize);
    }
    offset = Math.ceil((offset + fileSize) / 512) * 512;
  }
  return null;
}

async function downloadFallback() {
  // Like @sentry/cli, ship checksums with the wrapper so the fallback download
  // can be checked against the release binaries without trusting the download.
  const expectedChecksum = require("../checksums.json")[packageName];
  if (!expectedChecksum) {
    throw new Error(`No release checksum found for ${packageName}@${version}`);
  }
  ...
  if (actualChecksum !== expectedChecksum) {
    throw new Error(`Checksum validation failed for ${packageName}@${version}`);
  }

  const fallbackBinaryPath = helper.getFallbackBinaryPath();
  fs.mkdirSync(path.dirname(fallbackBinaryPath), { recursive: true });
  const temporaryPath = `${fallbackBinaryPath}.${process.pid}.tmp`;
  try {
    fs.writeFileSync(temporaryPath, binary, { mode: 0o755 });
    fs.renameSync(temporaryPath, fallbackBinaryPath);
  } finally {
...
The bt CLI will not be available.`,
  );
  process.exit(1);
});
