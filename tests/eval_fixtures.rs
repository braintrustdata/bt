...
fn eval_runner_list_mode_serializes_parameter_defaults() {
    ...
    // The default `.ts` runner is generated by bt at materialization time
    // (impl body + appended fire-and-forget). Reproduce that here so we can
    // hand a standalone script to ts-node without depending on the in-cwd
    // cache from a prior bt run.
    let impl_source = fs::read_to_string(root.join("scripts").join("eval-runner-impl.ts"))
        .expect("read eval-runner-impl.ts");
    let standalone = format!(
        "{impl_source}\nmain().catch((err) => {{\n  console.error(err);\n  process.exit(1);\n}});\n"
    );
    let runner_script = fixture_dir.join(".bt").join("test-eval-runner.ts");
    fs::create_dir_all(runner_script.parent().expect("parent")).expect("create .bt dir");
    fs::write(&runner_script, &standalone).expect("write standalone runner");
    ...
    let output = Command::new(&runner)
        ...
        .output()
        .expect("run eval-runner in list mode");
...
fn collect_deno_eval_diagnostics(dir: &Path, files: &[String]) -> Option<String> {
    ...
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let impl_source = fs::read_to_string(root.join("scripts").join("eval-runner-impl.ts")).ok()?;
    // Match what bt materializes for non-vite-node runners: impl body with
    // fire-and-forget appended. Standalone, no relative import to resolve.
    let standalone = format!(
        "{impl_source}\nmain().catch((err) => {{\n  console.error(err);\n  process.exit(1);\n}});\n"
    );
    ...
    fs::write(&local_runner, &standalone).ok()?;
