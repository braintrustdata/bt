    ...
    if let Some(new_slug) = common.new_slug.as_deref() {
        if new_slug != function.slug {
            if let Some(conflict) =
                api::get_function_by_slug(&ctx.client, &ctx.project.id, new_slug, None, None)
                    .await?
            {
...
async fn resolve_target_function(
    ...
    match args.selector()? {
        UpdateSelector::Id(id) => api::get_function_by_id(&ctx.client, id, None, None)
            .await?
            .ok_or_else(|| anyhow!("{} with id '{id}' not found", label(ft))),
        UpdateSelector::Slug(Some(slug)) => {
            api::get_function_by_slug(&ctx.client, project_id, slug, None, None)
                .await?
