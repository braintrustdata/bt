...
const VIEWS_PREVIEW_HTML_TEMPLATE: &str = include_str!("../scripts/custom-views-preview.html");
...
const JSX_RUNTIME_MODULE_SOURCE: &str = r#"const ReactValue = globalThis.React || React;
...
#[command(after_help = "\
Examples:
  bt custom-views push ./views
  bt custom-views push ./conversation.view.tsx --if-exists replace
  ...
  bt custom-views trace preview ./conversation.trace-view.tsx --trace-id <ROOT_SPAN_ID>
  bt custom-views dataset preview ./dataset.dataset-view.tsx --dataset test-dataset --row-index 0
")]
pub struct CustomViewsArgs {
    #[command(subcommand)]
...
enum ViewsCommands {
    ...
    Trace(TraceCustomViewsArgs),
    /// Work with dataset custom views
    Dataset(DatasetCustomViewsArgs),
}
...
pub async fn run(base: BaseArgs, args: CustomViewsArgs) -> Result<()> {
    match args.command {
...
fn write_bootstrap_scaffold(
    ...
    let (view_path, tsconfig_path) =
        if selected_path.is_dir() || selected_path.extension().is_none() {
            (
                selected_path.join(default_file_name),
                selected_path.join(CUSTOM_VIEW_TSCONFIG_FILE),
            )
        } else {
            let tsconfig_path = selected_path
                .parent()
                .unwrap_or_else(|| Path::new(""))
                .join(CUSTOM_VIEW_TSCONFIG_FILE);
            (selected_path, tsconfig_path)
        };
...
fn print_bootstrap_result(
    ...
    let result = BootstrapResult {
        ...
        tsconfig_created: matches!(result.tsconfig_status, BootstrapSupportFileStatus::Created),
        view_type,
    };
    if json_output {
        ...
        println!("Install dependencies: npm install braintrust@^3.33.0 react@^18 && npm install -D @types/react@^18");
        println!(
            "Preview it with: bt custom-views {} preview {}",
            view_type.label(),
            result.path
        );
    }
...
async fn push(base: BaseArgs, args: ViewsPushArgs) -> Result<()> {
    ...
    let prepared = prepare_views(&auth_ctx.client, default_project.as_ref(), &manifest).await?;
    ...
    let pushed = resolve_pushed_views(
        &auth_ctx.app_public_url,
    ...
    ui::print_command_status(
        ...
        &format!("Pushed {} custom view(s)", pushed.len()),
    );
    for view in pushed.iter().filter(|_| !ui::is_quiet()) {
...
async fn serve_preview(
    ...
    let state = web::Data::new(PreviewServerState {
        ...
        trace_source_expr: Mutex::new(None),
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            .service(
                web::scope("")
                    .guard(preview_request_guard(host.clone(), state.token.clone()))
                    .route("/", web::get().to(preview_index))
                    .route("/preview-assets/{asset}.js", web::get().to(preview_asset))
                    .route("/preview-module.js", web::get().to(preview_module))
...
fn preview_request_guard(host: String, token: String) -> impl guard::Guard {
    let origin = format!("http://{host}");
    guard::fn_guard(move |ctx| {
        let head = ctx.head();
        // Check the actual Host header, never forwarded headers: a loopback bind
        // alone does not stop a website from reaching us through DNS rebinding.
        head.headers
            .get("host")
            ...
            && head
                .headers
                .get("origin")
                .is_none_or(|value| value == origin.as_str())
            && head
                .headers
                .get("sec-fetch-site")
                .is_none_or(|value| value == "same-origin" || value == "none")
            && (head.method == actix_web::http::Method::GET
                || head
                    .headers
                    .get("x-bt-preview-token")
                    .is_some_and(|value| value == token.as_str()))
    })
}

async fn preview_index(state: web::Data<PreviewServerState>) -> HttpResponse {
    let bundle = match swc_bundle_custom_view(&state.source.path, SwcBundleTarget::Browser) {
        Ok(bundle) => bundle,
        Err(err) => {
            // Keep the preview page and its polling alive while a source edit fails to compile.
    ...
    let version = preview_source_version(&state.source, &bundle.dependency_paths);
    *state.bundle.lock().expect("preview bundle lock poisoned") = bundle;
    let html = render_preview_html(&state.title, &state.source, &version, &state.token);
    HttpResponse::Ok()
...
async fn preview_module(state: web::Data<PreviewServerState>) -> HttpResponse {
    let bundle = state.bundle.lock().expect("preview bundle lock poisoned");
    let expression = bundle.code.trim().trim_end_matches(';');
    HttpResponse::Ok()
        .insert_header(("Cache-Control", "no-store"))
        .content_type("application/javascript; charset=utf-8")
        .body(format!(
...
async fn preview_version(state: web::Data<PreviewServerState>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(json!({
        "version": preview_source_version(&state.source, &bundle.dependency_paths),
    }))
}
...
async fn preview_span_fields(
    ...
    let source_expr = match state.trace_source_expr.lock() {
        Ok(source_expr) => source_expr.clone(),
        Err(_) => None,
    };
    ...
    for span_id in &body.span_ids {
        ...
        match fetch_full_span_row(&state.client, &source_expr, &row_id).await {
            Ok(Some(row)) => {
                response.insert(
                    span_id.clone(),
    ...
    match &state.target {
        PreviewTarget::Trace(args) => {
            let project = if args.project_id.is_some()
                || trace_url_supplies_project(ViewType::Trace, args.url.as_deref())
            {
                None
            } else {
                Some(
                    resolve_project_for_entry(&state.client, state.project.as_ref(), &entry)
                        .await?,
                )
            };
            let trace_data =
                build_trace_preview_data(&state.client, project.as_ref(), args).await?;
            if let Ok(mut source_expr) = state.trace_source_expr.lock() {
                *source_expr = Some(trace_data.source_expr.clone());
            }
        ...
        PreviewTarget::Dataset(args) => {
            let project =
                resolve_project_for_entry(&state.client, state.project.as_ref(), &entry).await?;
...
fn render_preview_html(title: &str, source: &PreviewSource, version: &str, token: &str) -> String {
    let config_json = script_json(&json!({
        "title": title,
        "version": version,
        "sourcePath": source.path.display().to_string(),
        "sourceModuleUrl": "/preview-module.js",
...
fn run_views_runner(files: &[PathBuf]) -> Result<ViewsManifest> {
    ...
    for (index, file) in files.iter().enumerate() {
        ...
        let discovery = swc_bundle_custom_view(&source_file, SwcBundleTarget::Discovery)?;
        ...
        let browser_entry = temp_dir
            .path()
            .join(format!("{index}-{safe_name}.browser-entry.ts"));
        std::fs::write(
            &browser_entry,
            format!(
        ...
        let browser = swc_bundle_custom_view(&browser_entry, SwcBundleTarget::Browser)?;

        bundled_files.insert(
            source_key.clone(),
            module_exports_from_swc_iife(&browser.code),
        );
    ...
    for file in &manifest.files {
        for entry in &file.entries {
            ...
            if !seen.insert((project.id.clone(), entry.slug.clone())) {
                bail!(
                    "duplicate custom view slug '{}' in project '{}'",
                    entry.slug,
                    project.name
                );
            }
...
fn format_views_insert_error(
    ...
    let retry_hint = if if_exists == IfExistsMode::Error {
        " If you are updating an existing custom view, rerun with `bt custom-views push --if-exists replace`."
    } else {
        ""
    };
...
fn resolve_pushed_views(
    ...
    for view in views {
        let result = results
            .iter()
            .find(|result| result.project_id == view.project.id && result.slug == view.entry.slug)
            .with_context(|| {
                format!(
                    "custom view insert succeeded, but its response omitted '{}' in project '{}'; check the published views before retrying",
                    view.entry.slug, view.project.name
                )
            })?;
        ...
        pushed.push(PushedView {
            ...
            url: custom_view_url(app_url, org_name, view, &result.id),
        });
    }
    Ok(pushed)
}
...
async fn build_dataset_preview_data(
    ...
    let row = if let Some(row_id) = args.row_id.as_deref() {
        ...
        let limit = index.checked_add(1).context("row index is too large")?;
...
async fn fetch_trace_rows(
    ...
    let query = format!(
        "select: * | from: {source_expr} spans | filter: root_span_id = {} | preview_length: 125 | sort: _pagination_key ASC | limit: {}",
...
mod tests {
    ...
    async fn trace_test_server() -> (ApiClient, actix_web::dev::ServerHandle) {
        ...
        let server = HttpServer::new(|| {
            App::new().route(
                ...
                web::post().to(|body: web::Json<Value>| async move {
                    ...
                    let found = if query.contains("filter: span_id = 'selected-span'") {
                        if !query.contains("created >= NOW() - INTERVAL 604800 SECOND") {
                            return HttpResponse::BadRequest()
                                .body("missing configured lookup window");
                        }
                        true
                    } else {
                        query.contains("filter: root_span_id = 'root-span'")
                            || query.contains("filter: id = 'row-child'")
                    };
                    HttpResponse::Ok().json(json!({
                        "data": if found {
                            vec![json!({"root_span_id": "root-span", "span_id": "selected-span"})]
                        } else {
                            vec![]
                        }
                    }))
        ...
        .workers(1)
        .listen(listener)
        .expect("start mock API")
        .run();
        let handle = server.handle();
        tokio::spawn(server);
        (test_client(&api_url), handle)
    }
    ...
    fn push_runner_bundles_tsx_with_swc() {
        ...
        std::fs::write(dir.path().join("label.json"), r#"{"label":"SWC bundled"}"#)
            .expect("write JSON dependency");
        std::fs::write(
            &dependency,
...
export default customTraceView(
  ...
  function SwcTraceView() {
    return <section>{jsx("strong", { children: label }, "test-key")}</section>;
  },
);
"#,
        )
        ...
        let manifest = run_views_runner(std::slice::from_ref(&path)).expect("views manifest");
        ...
        let rendered: Value = serde_json::from_str(stdout.lines().last().expect("rendered output"))
            .expect("rendered component");
        assert_eq!(rendered["props"]["children"]["key"], "test-key");
        assert_eq!(
    ...
    #[test]
    fn push_runner_keeps_same_named_files_distinct() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut paths = Vec::new();
        for name in ["first", "second"] {
            let folder = dir.path().join(name);
            std::fs::create_dir(&folder).expect("create folder");
            let path = folder.join("test.dataset-view.tsx");
            std::fs::write(&path, format!(
                r#"import {{ customDatasetView }} from "braintrust/custom-views";
export default customDatasetView(
  {{ name: "{name}", slug: "{name}", project: "test-project", dataset: {{ name: "test-dataset" }} }},
  ({{ input, update }}) => <button onClick={{() => update({{ input }})}}>{name}</button>,
);"#
            )).expect("write view");
            paths.push(path);
        }
        let manifest = run_views_runner(&paths).expect("manifest");
        let first = &manifest.files[0].entries[0];
        let second = &manifest.files[1].entries[0];
        assert_eq!(first.slug, "first");
        assert_eq!(second.slug, "second");
        assert_eq!(first.view_type, ViewType::Dataset);
        assert_eq!(first.project_name.as_deref(), Some("test-project"));
        assert_eq!(first.dataset_name.as_deref(), Some("test-dataset"));
        assert_ne!(first.code, second.code);
    }

    #[test]
    fn preview_bundles_shared_and_package_imports_and_tracks_dependencies() {
        let dir = tempfile::tempdir().expect("tempdir");
        let views = dir.path().join("views");
        let components = dir.path().join("components");
        let package = dir.path().join("node_modules/test-view-label");
        for path in [&views, &components, &package] {
            std::fs::create_dir_all(path).expect("create fixture directory");
        }
        std::fs::write(
            package.join("package.json"),
            r#"{"name":"test-view-label","main":"index.js"}"#,
        )
        .expect("package manifest");
        let package_source = package.join("index.js");
        std::fs::write(&package_source, "export const label = 'shared label';")
            .expect("package source");
        let component = components.join("index.tsx");
        std::fs::write(
            &component,
            "import { label } from 'test-view-label';\nexport default () => <div>{label}</div>;",
        )
        .expect("shared component");
        let path = views.join("test.trace-view.tsx");
        std::fs::write(
            &path,
            "import { customTraceView } from 'braintrust/custom-views';\nimport Component from '../components';\nexport default customTraceView({ name: 'Test View', slug: 'test-view' }, Component);",
        )
        .expect("view source");
        let source = prepare_preview_source(&path, None, ViewType::Trace).expect("preview source");
        let bundle =
            swc_bundle_custom_view(&source.path, SwcBundleTarget::Browser).expect("preview bundle");
        for path in [&component, &package_source] {
            assert!(bundle
                .dependency_paths
        ...
        let output = Command::new("node")
            .arg("-e")
            .arg(format!(
                "globalThis.React = require('react');\n{}\nconsole.log(module.exports.component().props.children);",
                module_exports_from_swc_iife(&bundle.code),
            ))
            .output()
            .expect("render preview bundle");
        assert!(
            output.status.success(),
            "{}",
            String::from_utf8_lossy(&output.stderr)
        ...
        let before = preview_source_version(&source, &bundle.dependency_paths);
        std::fs::write(
            &package_source,
            "export const label = 'updated shared label';",
        )
        .expect("update package dependency");
        assert_ne!(
            before,
            preview_source_version(&source, &bundle.dependency_paths)
        );
    }

    #[test]
    fn push_discovers_wrapped_react_components() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("test.trace-view.tsx");
        std::fs::write(
            &path,
            "import { customTraceView } from 'braintrust/custom-views';\nimport { memo, forwardRef } from 'react';\nexport default customTraceView({ name: 'Test View', slug: 'test-view' }, memo(forwardRef(() => <div>wrapped</div>)));",
        )
        .expect("view source");
        let manifest = run_views_runner(&[path]).expect("discover wrapped component");
        assert_eq!(manifest.files[0].entries[0].slug, "test-view");
    }

    fn test_preview_source(path: PathBuf) -> PreviewSource {
        PreviewSource {
            path,
            view: None,
            view_type: ViewType::Trace,
    ...
    #[actix_web::test]
    async fn preview_rejects_untrusted_hosts_origins_and_missing_tokens() {
        ...
        let app = test::init_service(
            App::new()
                .service(
                    web::scope("")
                        .guard(preview_request_guard(
                            "127.0.0.1:8080".to_string(),
                            "test-token".to_string(),
                        ))
                        .route("/", web::to(|| async { HttpResponse::Ok().finish() })),
                )
                .default_service(web::to(|| async { HttpResponse::Forbidden().finish() })),
        )
        .await;
        for (method, host, origin, site, token, expected) in [
            (Method::GET, "127.0.0.1:8080", None, Some("none"), None, 200),
            (Method::GET, "attacker.example:8080", None, None, None, 403),
            (Method::GET, "127.0.0.1:8081", None, None, None, 403),
            (
                Method::GET,
                "127.0.0.1:8080",
                None,
            ...
            (Method::POST, "127.0.0.1:8080", None, None, None, 403),
            (
                Method::POST,
                "127.0.0.1:8080",
                None,
            ...
            let mut request = test::TestRequest::default()
                ...
                .insert_header(("x-forwarded-host", "127.0.0.1:8080"));
            for (name, value) in [
                ("origin", origin),
                ("sec-fetch-site", site),
                ("x-bt-preview-token", token),
            ] {
    ...
    fn preview_html_includes_hot_reload_polling() {
        ...
        let source = test_preview_source(path);
        let html = render_preview_html("Preview", &source, "v1", "test-token");
        ...
        assert!(html.contains("/preview-module.js"));
    ...
    fn preview_source_version_changes_when_source_changes() {
        ...
        let path = dir.path().join("test.trace-view.tsx");
        std::fs::write(&path, "export default null;\n").expect("write view");
        let source = test_preview_source(path.clone());
    ...
    #[actix_web::test]
    async fn experiment_preview_uses_experiment_for_trace_and_field_queries() {
        let listener = TcpListener::bind(("127.0.0.1", 0)).expect("bind mock API");
        ...
        let server = HttpServer::new(|| {
            App::new()
                .route(
                    "/v1/experiment",
                    web::get().to(|| async {
                        HttpResponse::Ok().json(json!({"objects": [{
                            "id": "exp_test",
                ...
                .route(
                    ...
                    web::post().to(|body: web::Json<Value>| async move {
                        ...
                        if !query.contains("from: experiment('exp_test')")
                            || query.contains("created >=")
                        {
                            return HttpResponse::BadRequest()
                                .body("wrong experiment source or window");
                        }
                        HttpResponse::Ok().json(json!({"data": [{
                            "id": "row-child",
                            "span_id": "selected-span",
                            "root_span_id": "root-span",
                            "input": "full experiment input",
                        }]}))
                    }),
                )
        })
        ...
        tokio::spawn(server);
        let state = web::Data::new(PreviewServerState {
            client: test_client(&api_url),
            project: None,
            target: PreviewTarget::Trace(TracePreviewTargetArgs {
                url: Some("https://app.example.com/app/test-org/p/00000000-0000-4000-8000-000000000001/experiments/test-experiment?s=selected-span".to_string()),
                project_id: None,
                trace_id: None,
                span_id: None,
                lookup_window: "30d".to_string(),
            }),
        ...
        .await
        .expect("experiment preview data");
        assert_eq!(data["span"]["id"], "row-child");
        let response = preview_span_fields(
            state,
            web::Json(SpanFieldsRequest {
                span_ids: vec!["selected-span".to_string()],
                fields: Some(vec!["input".to_string()]),
            }),
        )
        .await;
        ...
        assert_eq!(fields["selected-span"]["input"], "full experiment input");
        handle.stop(true).await;
    }

    #[tokio::test]
    async fn trace_preview_url_project_id_does_not_require_default_project() {
        let (client, server) = trace_test_server().await;
        ...
        assert_eq!(target.source_expr, format!("project_logs('{project_id}')"));
    ...
    async fn trace_preview_url_without_project_uses_default_project() {
        ...
        assert_eq!(target.source_expr, "project_logs('proj_test')");
        assert_eq!(target.root_span_id, "root-span");
        assert_eq!(target.span_id.as_deref(), Some("selected-span"));
        server.stop(true).await;
    }
    ...
    #[tokio::test]
    async fn trace_preview_resolves_row_and_span_urls_with_encoded_project_names() {
        let (client, server) = trace_test_server().await;
        let mut project = test_project();
        project.name = "test project".to_string();
        for query in ["r=row-child", "s=selected-span"] {
            let target = resolve_trace_preview_target(
                &client,
                Some(&project),
                &TracePreviewTargetArgs {
                    url: Some(format!(
                        "https://www.braintrust.dev/app/test-org/p/test%20project/logs?{query}"
                    )),
                    project_id: None,
                    trace_id: None,
                    span_id: None,
                    lookup_window: "7d".to_string(),
                },
            )
            .await
            .expect("resolve trace URL");
            assert_eq!(
                target.source_expr,
                format!("project_logs('{}')", project.id)
            );
            assert_eq!(target.root_span_id, "root-span");
    ...
    fn formats_actionable_custom_view_insert_error() {
        ...
        assert!(message.contains("bt custom-views push --if-exists replace"));
        assert!(message.contains("slug already exists"));
    }

    #[test]
    fn push_reporting_distinguishes_ignored_views_from_replacements() {
        let views = ["existing-view", "new-view"].map(|slug| PreparedView {
            source_file: format!("{slug}.view.tsx"),
            entry: ViewManifestEntry {
                view_type: ViewType::Trace,
                name: slug.to_string(),
                slug: slug.to_string(),
        ...
        for (mode, expected) in [(IfExistsMode::Ignore, 1), (IfExistsMode::Replace, 2)] {
            ...
            assert!(pushed.last().unwrap().url.contains("tv=fn_test_new"));
        }
    }
