//! `bt daemon` — embeds the `bt-daemon` tracing daemon as subcommands.
//!
...
//! `resolve_auth` (profiles / OAuth refresh / keychain), so a `bt daemon hook`
...
/// How the shim (re)launches the daemon: `bt daemon serve` from this same
/// binary.
fn host_info() -> HostInfo {
    ...
    HostInfo {
        serve_argv: vec![exe, OsString::from("daemon"), OsString::from("serve")],
...
pub async fn run(base: BaseArgs, args: DaemonArgs) -> anyhow::Result<()> {
    match args.command {
        DaemonCommand::Serve(serve_args) => run_serve(serve_args, serve_options()).await,
        DaemonCommand::Hook(hook_args) => {
            ...
            match session_config(&base).await {
                ...
                Err(e) => eprintln!("bt daemon hook (non-fatal): {e}"),
            }
            Ok(())
        }
        DaemonCommand::Status(status_args) => match run_status(status_args).await? {
            Some(status) => {
        ...
        DaemonCommand::Replay(replay_args) => {
