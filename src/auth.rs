...
pub async fn list_available_orgs(base: &BaseArgs) -> Result<Vec<AvailableOrg>> {
    ...
    let mut orgs = fetch_login_orgs(&api_key, &app_url, base.ca_bundle.as_deref()).await?;
    orgs.sort_by(|a, b| {
        a.name
            .to_ascii_lowercase()
...
pub async fn login(base: &BaseArgs) -> Result<LoginContext> {
    ...
    if let Some(ca_bundle) = &base.ca_bundle {
        builder = builder.ca_bundle(ca_bundle.clone());
    }
    ...
    Ok(LoginContext {
        ...
        api_url,
        app_url,
        ca_bundle: base.ca_bundle.clone(),
    })
}
...
pub async fn resolve_auth(base: &BaseArgs) -> Result<ResolvedAuth> {
    ...
    let refreshed = refresh_oauth_access_token(
        &api_url,
        &refresh_token,
        client_id,
...
async fn run_login_set(base: &BaseArgs, args: AuthLoginArgs) -> Result<()> {
    ...
    let login_app_url = base
        ...
        .clone()
        .unwrap_or_else(|| DEFAULT_APP_URL.to_string());
    let login_orgs = fetch_login_orgs(&api_key, &login_app_url, base.ca_bundle.as_deref()).await?;
...
async fn run_login_oauth(base: &BaseArgs, args: AuthLoginArgs) -> Result<()> {
    ...
    .await?;
    let login_orgs = fetch_login_orgs(
...
async fn run_profiles(base: &BaseArgs, args: AuthProfilesArgs) -> Result<()> {
    ...
    let verifications = verify_all_profiles_from_store(&store, base.ca_bundle.clone()).await;
...
async fn verify_profile_full(
    ...
    match fetch_login_orgs(&credential, app_url, ca_bundle.as_deref()).await {
...
async fn verify_all_profiles_from_store(
    ...
    for (name, profile) in store.profiles.iter() {
        ...
        set.spawn(async move { verify_profile_full(&name, &profile, ca_bundle).await });
    }
...
async fn fetch_login_orgs(
    ...
    let client = build_http_client(crate::http::DEFAULT_HTTP_TIMEOUT, ca_bundle)
...
async fn exchange_oauth_authorization_code(
    ...
    let token_response = oauth_client
        ...
        .request_async(move |request| oauth_async_http_client(request, http_client))
        .await
        .with_context(|| {
            format!(
...
#[derive(Debug)]
struct OAuthHttpClientError(String);

impl std::fmt::Display for OAuthHttpClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.0)
    }
}

impl std::error::Error for OAuthHttpClientError {}

async fn oauth_async_http_client(
    request: oauth2::HttpRequest,
    client: reqwest::Client,
) -> std::result::Result<oauth2::HttpResponse, OAuthHttpClientError> {
    let method = reqwest::Method::from_bytes(request.method.as_str().as_bytes())
        .map_err(|err| OAuthHttpClientError(format!("invalid oauth request method: {err}")))?;
    ...
    let request = request_builder
        .build()
        .map_err(|err| OAuthHttpClientError(format!("failed to build oauth request: {err}")))?;

    let response = client
        .execute(request)
        .await
        .map_err(|err| OAuthHttpClientError(format!("failed to send oauth request: {err}")))?;
    let status_code = oauth2::http::StatusCode::from_u16(response.status().as_u16())
        .map_err(|err| OAuthHttpClientError(format!("invalid oauth response status: {err}")))?;
    let mut headers = oauth2::http::HeaderMap::with_capacity(response.headers().len());
    for (name, value) in response.headers() {
        let name = oauth2::http::header::HeaderName::from_bytes(name.as_str().as_bytes()).map_err(
            |err| OAuthHttpClientError(format!("invalid oauth response header name: {err}")),
        )?;
        let value = oauth2::http::HeaderValue::from_bytes(value.as_bytes()).map_err(|err| {
            OAuthHttpClientError(format!("invalid oauth response header value: {err}"))
        })?;
    ...
    let body = response
        .bytes()
        .await
        .map_err(|err| OAuthHttpClientError(format!("failed to read oauth response body: {err}")))?
        .to_vec();
